...
wifi.active(True) 
wifi.connect(ssid,ssid_passwd) 
time.sleep(8)

i2c =  I2C(scl=Pin(0), sda=Pin(4), freq=100000)
...
baseurl = server + '/api/v1/quote?symbol=BINANCE:%s&token=%s' 
...
def rr(name,tag):
    ...
    if h+16<=24:
        ...
        h=h-16
    t=formats(str(h))+':'+formats(str(utime.localtime()[4]))+':'+formats(str(utime.localtime()[5]))
    oled.fill(0)
    oled.text(name,0,1)
    #oled.text(t, 2, 35)
    chinese4(t,0,2)
